...
function analytics_beacon_injector_start(): void
{
    ...
    $searchCss = $script === 'search.php'
        ? (function_exists('asset_url') ? asset_url('css/search-results.css') : '/assets/css/search-results.css')
        : '';
    ...
    $headTags = '<meta name="referrer" content="unsafe-url">';
    if ($searchCss !== '') {
        $headTags .= '<link rel="stylesheet" href="' . htmlspecialchars($searchCss, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . '">';
    }

    ob_start(static function (string $html) use ($pvJs, $engagementTag, $headTags): string {
        if ($html === '') return $html;
        if (str_contains($html, '</head>')) {
            $insert = $headTags;
            if (str_contains($html, 'name="referrer"')) {
                $insert = preg_replace('/<meta name="referrer" content="unsafe-url">/', '', $insert, 1) ?? $insert;
            }
            $html = str_replace('</head>', $insert . '</head>', $html);
        }
