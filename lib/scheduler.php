...
function scheduler_run_schedule(array $schedule): array
{
    ...
    return match ($type) {
        ...
        'actresses' => scheduler_run_master_schedule('actresses', sokumiru_sync_service('actresses'), $settings),
...
function scheduler_run_master_schedule(string $jobKey, SokumiruSyncService $service, array $settings): array
{
    $pdo = db();
    scheduler_ensure_job_state_table($pdo);
