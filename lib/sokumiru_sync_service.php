...
class SokumiruSyncService
{
    ...
    public function syncMaster(string $kind, ?string $floorId = null, int $offset = 1, int $hits = 100, array $extraParams = []): int
    {
        ...
        $rows = SokumiruNormalizer::toList($response['result'][$key] ?? []);
        $table = match ($kind) {
            'genre' => 'genres',
            'maker' => 'makers',
            'author' => 'authors',
        ...
        try {
            foreach ($rows as $r) {
                ...
                $stmt->execute([
                    ...
                    'img' => is_string($r['imageURL'] ?? null) ? $r['imageURL'] : ($r['imageURL']['large'] ?? ($r['image_url'] ?? null)),
                    'img_small' => is_string($r['imageURL'] ?? null) ? $r['imageURL'] : ($r['imageURL']['small'] ?? ($r['image_small'] ?? null)),
                    'img_large' => is_string($r['imageURL'] ?? null) ? $r['imageURL'] : ($r['imageURL']['large'] ?? ($r['image_large'] ?? null)),
                ]);
                $count++;
            }
            $this->pdo->commit();
