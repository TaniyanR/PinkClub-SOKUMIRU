...
if ($name === '' || $dmmId === '' || str_starts_with($dmmId, 'name:')) {
    actress_profile_json_response(['success' => false], 404);
}
...
try {
    $client = sokumiru_client_for_type('actresses');
    $response = $client->searchActresses(['keyword' => $name, 'hits' => 20, 'offset' => 1]);
    $apiRows = SokumiruNormalizer::toList($response['result']['actor'] ?? []);
    ...
    if (!is_array($bestApiRow)) {
        ...
        $keywordRows = SokumiruNormalizer::toList($keywordResponse['result']['actor'] ?? []);
    ...
    if (is_array($bestApiRow)) {
        ...
        $apiImage = is_string($bestApiRow['imageURL'] ?? null) ? $bestApiRow['imageURL'] : '';
        $profile['image_url'] = $apiImage !== '' ? $apiImage : (string)($bestApiRow['imageURL']['large'] ?? $bestApiRow['image_url'] ?? $profile['image_url']);
        $profile['image_small'] = $apiImage !== '' ? $apiImage : (string)($bestApiRow['imageURL']['small'] ?? $bestApiRow['image_small'] ?? $profile['image_small']);
        $profile['image_large'] = $apiImage !== '' ? $apiImage : (string)($bestApiRow['imageURL']['large'] ?? $bestApiRow['image_large'] ?? $profile['image_large']);
