...
function auto_import_lock()
{
    ...
    if (!is_dir($lockDirectory) && !@mkdir($lockDirectory, 0775, true) && !is_dir($lockDirectory)) throw new RuntimeException('cronロック用ディレクトリを作成できません');
    $handle = @fopen($lockDirectory . '/auto-import.lock', 'c');
    if (!is_resource($handle)) throw new RuntimeException('cronロックファイルを開けません');
...
if (PHP_SAPI === 'cli' && realpath((string)($_SERVER['SCRIPT_FILENAME'] ?? '')) === __FILE__) exit(main());
